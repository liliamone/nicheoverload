    ...
    public function register_settings()
    {
        ...
        foreach ($this->options as $id => $field)
        {
            ...
            if (!empty($field['autofocus']))
                $params['autofocus'] = $field['autofocus'];
            if (!empty($field['help_url']))
                $params['help_url'] = $field['help_url'];
            if (!empty($field['aria_expanded']))
                $params['aria_expanded'] = $field['aria_expanded'];
            if (!empty($field['required']))
                $params['required'] = $field['required'];
            if (!empty($field['maxlength']))
                $params['maxlength'] = $field['maxlength'];
            if (!empty($field['height']))
                $params['height'] = $field['height'];

            $params['is_invalid'] = $this->isInvalid($id);
                ...
                foreach ($validator as $v)
                {
                    ...
                    else
                    {
                        ...
                        if (!empty($v['when']))
                        {
                            if (is_array($v['when']))
                            {
                                $when_value = $this->get_submitted_value($v['when'][0]);

                                if ($when_value != $v['when'][1])
                                    continue;
                            }
                            else
                            {
                            ...
                            if (!$res)
                            {
                                ...
                                if (!empty($v['when']))
                                {
                                    if (is_array($v['when']))
                                        ...
                                        $when = $v['when'];
                                    $this->out[$when] = $this->get_current($when);
                                }
                                break;
                            }
                        } // .validator
